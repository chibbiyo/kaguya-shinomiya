...
export default class Command extends BaseCommand {
    constructor(client: WAClient, handler: MessageHandler) {
        super(client, handler, {
            command: 'sticker',
            aliases: ['s'],
            description: 'Converts images/videos into stickers. \nThat here --c crops the sticker into square and --s stretches it.\nIf normal ${client.config.prefix}sticker command does not work because the gif is too big. \nYou can try ${client.config.prefix}sticker with lower quality.\nlike ${client.config.prefix}sticker --1 or vice versa.\n*Note:* \nNumber must be between 1-100 else it will be set to default 50 value',
            category: 'utils',
